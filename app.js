...
async function cacheSheetData(data) {
  ...
  for (const r of (data.stockOut || [])) {
    ...
    const rec = {
      ...
      penyebab: r.penyebab || "", catatan: r.catatan || "", created_at: r.created_at || "", synced: true, source: "sheet"
    };
...
/* ───────────── Excel stock baseline ─────────────
   Source workbook: Database Apar Update 24042026(Terbaru) (1).xlsx
   Resume calculation at migration snapshot:
     APAR 6KG: IN 1,751 - OUT 1,725 = 26
     APAR 3KG: IN   718 - OUT   610 = 108

   Google Sheets becomes the transaction ledger after this cutover.
   Excel rule is preserved exactly:
     Stock IN  = SUM(jumlah)
     Stock OUT = COUNT(rows), one row = one APAR
 */
const EXCEL_STOCK_BASELINE = Object.freeze({
  cutoverAt: "2026-08-21T16:26:00+07:00",
  sixKg: 26,
  threeKg: 108,
});

function stockKind(value) {
  const v = String(value || "").toUpperCase().replace(/\s+/g, "").replace(/APAR/g, "");
  if (v === "6" || v === "6KG" || /(^|[^0-9])6KG($|[^0-9])/.test(v)) return "6kg";
  if (v === "3" || v === "3KG" || /(^|[^0-9])3KG($|[^0-9])/.test(v)) return "3kg";
  return "";
}

function isPostExcelCutover(record) {
  const raw = record?.created_at || record?.createdAt || record?.syncedAt || "";
  if (!raw) return false; // historical rows are already represented by the Excel baseline
  const t = new Date(raw).getTime();
  const cut = new Date(EXCEL_STOCK_BASELINE.cutoverAt).getTime();
  return Number.isFinite(t) && t > cut;
}

function calculateExcelStock(stockInRows = [], stockOutRows = []) {
  let in6 = 0, in3 = 0, out6 = 0, out3 = 0;

  for (const r of stockInRows) {
    if (!isPostExcelCutover(r)) continue;
    const kind = stockKind(r.jenis || r.type || r.kapasitas);
  ...
  for (const r of stockOutRows) {
    if (!isPostExcelCutover(r)) continue;
    const kind = stockKind(r.jenis || r.type || r.kapasitas);
    // Excel OUT logic: one matching transaction row = one APAR.
    if (kind === "6kg") out6 += 1;
    if (kind === "3kg") out3 += 1;
  }

  return {
    sixKg: EXCEL_STOCK_BASELINE.sixKg + in6 - out6,
    threeKg: EXCEL_STOCK_BASELINE.threeKg + in3 - out3,
    movement: { in6, in3, out6, out3 }
  };
}
...
async function renderDashboard(snapshot = serverSnapshot) {
  ...
  // Match the Excel Resume logic exactly. Historical Excel movement is represented
  // by the migration baseline; only transactions created after cutover adjust it.
  const stock = calculateExcelStock(sIn, sOut);
  document.getElementById("dash6kg").textContent = stock.sixKg;
  document.getElementById("dash3kg").textContent = stock.threeKg;
...
async function renderStokSummary() {
  let sIn, sOut;
  if (serverSnapshot && serverSnapshot.ok !== false) {
    sIn = Array.isArray(serverSnapshot.stockIn) ? serverSnapshot.stockIn : [];
    sOut = Array.isArray(serverSnapshot.stockOut) ? serverSnapshot.stockOut : [];
  } else {
    [sIn, sOut] = await Promise.all([dbGetAll("stock_in"), dbGetAll("stock_out")]);
  }
  const stock = calculateExcelStock(sIn, sOut);
  document.getElementById("stok6kg").textContent = stock.sixKg;
  document.getElementById("stok3kg").textContent = stock.threeKg;
}
